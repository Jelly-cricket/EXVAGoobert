...
public partial class PlayerInputSignals : CharacterInputSignals
{
	public override void _Input(InputEvent e)
	{
		...
		if (e.IsActionPressed("kit_special_equip"))
			EmitSignal(SignalName.KitSpecialEquipPressed);

		if (e.IsActionReleased("kit_special_equip"))
			EmitSignal(SignalName.KitSpecialEquipReleased);

		if (e.IsActionPressed("kit_special_deploy"))
			EmitSignal(SignalName.KitSpecialDeployPressed);

		if (e.IsActionReleased("kit_special_deploy"))
			EmitSignal(SignalName.KitSpecialDeployReleased);
