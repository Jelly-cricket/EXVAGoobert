...
public partial class StatComponent : BaseComponent
{
	...
	public float Amount
	{
		get => _amount; private set => _amount = value;
	}
	...
	public bool CanAbsorbAmount(float desired) => Amount + desired <= MaxAmount;

	public bool CanDrainAmount(float desired) => Amount - desired >= MinAmount;
	...
	public virtual void AbsorbAmount(float desired) => SetAmount(Amount + desired);
	...
	private void TickRegenCooldown(double delta) => _regenCooldown = Mathf.Max(_regenCooldown - delta, 0);
